...
function FeaturedProdDetails() {
  const {cartHandler,isDisabled,data,setData} = useContext(stateHandler)
  ...
  useEffect (()=>{
      const apiFetcher = async(req, res)=>{
          try{
              const response = await fetch(`http://localhost:5000/featuredProducts/${search.id}`)
